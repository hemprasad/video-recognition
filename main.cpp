//////////////////////////////////////////////////////////////////////////////////////
// Based on OnlineFaceRec.cpp, by Shervin Emami (www.shervinemami.co.cc) on 2nd June 2010.
...
const char *faceCascadeFilename = "C:\\Programs\\OpenCV2.2\\data\\haarcascades\\haarcascade_frontalface_default.xml";
...
int haarZoom = 20, haarLoops = 2, haarDivision = 10;
cqueue<string> recentPersons(60);
...
IplImage* InitializeCam(int num);
void logRecognizedPerson(string person, time_t curTime = NULL);
...
{
	printf("Acessing the camera ...\n");
	VI = new videoInputCam(num);
	if (!VI) {
		return 0;
	}
	// Try to set the camera resolution
	//VI->SetRecognition(320, 240);
	VI->SetRecognition(1024, 648);
	// Wait a little, so that the camera can auto-adjust itself
	Sleep(1000);	// (in milliseconds)
	IplImage *frame = VI->GetFrame();	// get the first frame, to make sure the camera is initialized.
	if (frame) {
		//printf("Got a camera using a resolution of %dx%d.\n", (int)cvGetCaptureProperty( camera, CV_CAP_PROP_FRAME_WIDTH), (int)cvGetCaptureProperty( camera, CV_CAP_PROP_FRAME_HEIGHT) );
		printf("Got a camera using a resolution of %dx%d.\n", VI->GetX(), VI->GetY() );
	}
...
CvRect detectFaceInImage(const IplImage *inputImg, const CvHaarClassifierCascade* cascade )
{
	//int minFaceSize = inputImg->width / 10;
	int minFaceSize = inputImg->width / (haarDivision + 1);
	//const CvSize minFeatureSize = cvSize(20, 20);
	const CvSize minFeatureSize = cvSize(minFaceSize, minFaceSize);
	...
	const float search_scale_factor = 1 + (float)haarZoom / 100;
	IplImage *detectImg;
	IplImage *greyImg = 0;
	...
	rects = cvHaarDetectObjects( detectImg, (CvHaarClassifierCascade*)cascade, storage,
				search_scale_factor, haarLoops, flags, minFeatureSize );
...
void recognizeFromCam(void)
{
	...
	printf("Loading the training data...\n");
	...
	cv::createTrackbar( "Zoom", "Input", &haarZoom, 100, 0 );
	cv::createTrackbar( "Loops", "Input", &haarLoops, 20, 0 );
	cv::createTrackbar( "Division", "Input", &haarDivision, 99, 0 );

	cvNamedWindow("TestFace", CV_WINDOW_AUTOSIZE);
	...
	// Initialize the camera and the tracker
	IplImage *frame = 0;
	//if(!VI)
	//{
	//	frame = InitializeCam(0);
	//}else
	//	frame = VI->GetFrame();
	//if(frame)
	//	//createTracker(frame);
	//	tracker = new Tracker(frame);
	...
	while (1)
	{
		...
		if (stopLearning || newPersonFaces >= 50)
		{
			stopLearning = false;
		...
		//	trackedFace = tracker->track(camImg);
		//	t = (double)cvGetTickCount() - t;
		//	printf("[Tracking took %d ms]\n", cvRound( t/((double)cvGetTickFrequency()*1000.0) ));
		//}
		...
		if (faceRect.width > 0) {
			...
			if (nEigens > 0) {
				...
				iNearest = findNearestNeighbor(projectedTestFace, &confidence);
				cvShowImage("TestFace", processedFaceImg);

				nearest  = trainPersonNumMat->data.i[iNearest];

				//Start tracking the face to find the match with highest confidence
				if(abs(confidence) < MIN_CONFIDENCE){
					//if(!tracking){
					//	tracker->startTracking(camImg, &faceRect);
					//	tracking = true;
					//} else {
					//	
					//}

					//If this person wasn't found on the last N images, log it into the file
					string foundPerson = personNames[nearest-1];
					if(!recentPersons.find(foundPerson))
						logRecognizedPerson(foundPerson);
					recentPersons.push(foundPerson);
				}
				else{
					recentPersons.push("");
				}
...
// MAYBE GET IT TO ONLY TRAIN SOME IMAGES ?
		...
		if (faceRect.width > 0) {	// Check if a face was detected.
			...
			//if (nEigens > 0 && abs(confidence) < MIN_CONFIDENCE) {	// Check if the face recognition database is loaded and a person was recognized.
			if (nEigens > 0 && abs(confidence) < MIN_CONFIDENCE) {	// Check if the face recognition database is loaded and a person was recognized.
		...
		//Draw the tracked face ellipse
		if(tracking && (trackedFace.center.x > 0 && trackedFace.center.y > 0))
			cvEllipseBox(shownImg, trackedFace,
...
//Write to the log file the name and the time of th recognized person
void logRecognizedPerson(string person, time_t curTime)
{
 	if(curTime == NULL)
		time(&curTime);
